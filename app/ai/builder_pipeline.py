...
async def gate_assemble(intake_res: Dict[str, Any], grounding_res: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fmt = intake_res.get("format", "double")
    user_msg = f"""【对战模式】：{intake_res['format_cn']} ({'6选4 双打 (VGC)' if fmt == 'double' else '6选3 单打 (Singles)'})
...
【严格限定的候选物种池 (仅能从以下物种中挑选 6 只，道具必须从对应 top_items 中选取)】：
{cand_str}

请输出装配好的 6 只宝可梦队伍 JSON (请确保 rationale 详细、真实且符合 {'6选4双打' if fmt == 'double' else '6选3单打'} 竞技逻辑)："""
